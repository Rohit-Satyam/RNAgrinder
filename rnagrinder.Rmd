...
```{r}
ribodetector <- plyr::ldply(lapply(list.files(path = "results/rRNA/ribodetector/",pattern = "*_ribodetector_results.txt*",full.names = TRUE), function(x){read.csv(x,header = F, col.names = c("Sample","Total-Reads","rRna","non-rRNA"), check.names = F)}))
...
temp <- list.files(path="results/02_adapterTrimming",pattern = "*fastp.txt", full.names =TRUE)[!grepl("L001",list.files(path="results/02_adapterTrimming",pattern = "*fastp.txt", full.names =TRUE))]
...
write.csv(df3,"saras_summary.csv")
```
